...
describe('angularjs homepage todo list', function() {
	it('should add a new contact', function() {
		...
		expect(todoList.get(2).element(by.model('contact.name')).getAttribute('value')).toBe('Ross');
		expect(todoList.get(2).element(by.model('contact.phone')).getAttribute('value')).toBe('0684059433');
	});
});
